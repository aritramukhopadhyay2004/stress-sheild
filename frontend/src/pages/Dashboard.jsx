...
export default function Dashboard({ token, setToken }) {
  ...
  const fetchData = async () => {
    try {
      const [historyRes, alertsRes] = await Promise.all([
        api.get('/api/health/history', { headers: { Authorization: `Bearer ${token}` } }),
  ...
  const setupSocket = () => {
    ...
    socket.on('stress-alert', (data) => {
      toast.error(`⚠️ ${data.stress_level} Stress Detected!`, { duration: 5000 });
      fetchData();
    });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-800">Stress-Shield Dashboard</h1>
          <p className="text-gray-600 mt-2">Monitor your stress levels in real-time</p>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-md p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                {latestReading ? (
                  <p className={`text-2xl font-bold mt-2 ${getStressColor(latestReading.stress_level).split(' ')[0]}`}>
          ...
          <div className="bg-white rounded-xl shadow-md p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-800 mt-2">{readings.length}</p>
              </div>
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {alerts.length > 0 ? (
                alerts.slice(0, 10).map((alert) => (
                  <div key={alert.id} className="flex items-start p-4 bg-red-50 rounded-lg border-l-4 border-red-500">
